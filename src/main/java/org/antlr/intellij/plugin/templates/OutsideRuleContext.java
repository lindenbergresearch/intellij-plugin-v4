...
public class OutsideRuleContext extends ANTLRLiveTemplateContext {
    ...
    public boolean isInContext(@NotNull PsiFile file, @NotNull PsiElement element, int offset) {
        var tokens = ParsingUtils.tokenizeANTLRGrammar(file.getText());
        var tokenUnderCursor = ParsingUtils.getTokenUnderCursor(tokens, offset);
        if (tokenUnderCursor == null) {
            return false; // sometimes happens at the eof
        }
        
        var tokenIndex = tokenUnderCursor.getTokenIndex();
        var nextRealToken = ParsingUtils.nextRealToken(tokens, tokenIndex);
        var previousRealToken = ParsingUtils.previousRealToken(tokens, tokenIndex);
        
        if (nextRealToken == null || previousRealToken == null) {
            return false;
        }
        
        var previousRealTokenType = previousRealToken.getType();
        var nextRealTokenType = nextRealToken.getType();
        
        if (previousRealTokenType == ANTLRv4Parser.BEGIN_ACTION) {
            // make sure we're not in a rule; has to be @lexer::header {...} stuff
            var prevPrevRealToken = ParsingUtils.previousRealToken(tokens, previousRealToken.getTokenIndex());
            if (prevPrevRealToken == null) {
                return false;
            }
            
            var prevPrevPrevRealToken = ParsingUtils.previousRealToken(tokens, prevPrevRealToken.getTokenIndex());
