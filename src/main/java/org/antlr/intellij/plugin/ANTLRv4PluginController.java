...
public class ANTLRv4PluginController implements ProjectComponent {
    ...
    public static void showConsoleWindow(final Project project) {
        ApplicationManager.getApplication().invokeLater(
            () -> Objects.requireNonNull(ANTLRv4PluginController.getInstance(project)).getConsoleWindow().show(null)
        );
    }
    ...
    public void currentEditorFileChangedEvent(VirtualFile oldFile, VirtualFile newFile) {
        ...
        if (previewState.grammar == null && previewState.lexerGrammar == null) { // only load grammars if none is there
    ...
    public void editorFileClosedEvent(VirtualFile vfile) {
        ...
        previewState.grammar = null; // wack old ref to the Grammar for text in editor
    ...
    public PreviewState getAssociatedParserIfLexer(String grammarFileName) {
        for (PreviewState s : grammarToPreviewState.values()) {
            if (s != null && s.lexerGrammar != null &&
                (sameFile(grammarFileName, s.lexerGrammar.fileName) || s.lexerGrammar == ParsingUtils.BAD_LEXER_GRAMMAR)) {
                // s has a lexer with same filename, see if there is a parser grammar
                // (not a combined grammar)
                if (s.grammar != null && s.grammar.getType() == ANTLRParser.PARSER) {
                    return s;
                }
            }
            
            if (s != null && s.grammar != null && s.grammar.importedGrammars != null) {
                for (Grammar importedGrammar : s.grammar.importedGrammars) {
        ...
        parsingProgressIndicator = BackgroundTaskUtil.executeAndTryWait(
            (indicator) -> {
                previewState.parsingResult = ParsingUtils.parseText(
                    previewState.grammar, previewState.lexerGrammar, previewState.startRuleName,
